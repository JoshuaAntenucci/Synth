...
def callback(outdata, frames, time, status):
    ...
    outdata[:] = sine_wave[int(time.outputBufferDacTime * samplerate):
                           int((time.outputBufferDacTime + frames/samplerate) * samplerate)].reshape(-1, 1)
...
def play_sound():
    ...
    with sd.OutputStream(samplerate=44100, channels=1, callback=callback) as stream:
        print(f"Playing sine wave for {3} seconds...")
        sd.sleep(int(5 * 1000))  # Keep the stream open for the duration
